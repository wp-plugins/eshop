	...
	switch($stock){
		case 'dloads':
			...
			foreach($mypages as $row){
				...
				echo '<tr'.$alt.'>';
				echo '<td id="redid'.$row->id.'" headers="edtitle'.$rand.'"><a href="?page=eshop-downloads.php&amp;edit='.$row->id.'" title="edit details for '.$row->title.'">'.esc_html(stripslashes($row->title),1)."</a></td>\n";
